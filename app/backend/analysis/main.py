...
def analysissentiment(ierjson, ieojson, mapreducejson):
    ...
    df = pd.DataFrame(columns=['SA2_id', 'ierscore', 'pop','ieoscore','sentiment'])

    for ierkey in ierjson:
        ierdata = ierkey['key']
        df = df.append([{'SA2_id': ierkey['id'], 'ierscore': ierdata[1], 'pop': ierdata[0]}])
    ...
    for ieokey in ieojson:
        ...
        try:
            ...
            print("area not exist",ieokey['id'])
            pass

    for mapkey in mapreducejson:
        try:
            happypercentage = (mapkey['positive'])/(mapkey['positive'] + mapkey['neutral'] +mapkey['negative'])
            df.loc[(df.SA2_id == mapkey['area']), 'sentiment'] = happypercentage
        except:
            pass
