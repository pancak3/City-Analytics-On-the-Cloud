    ...
    for (const ddoc_name of ddoc_names) {
        ...
        if (stats.isDirectory()) {
            ...
            for (const view_name of view_names) {
                file_api_views[ddoc_name][view_name] = readMapReduce(
                    path.join(ddoc_path, view_name)
                );
            }
        }
    }
...
async function getCouchDDocs(fileViews) {
    ...
    const users_ddocs_res = await axios.get(
        `${base_url}/users/_all_docs?inclusive_end=false&start_key=%22_design%22&end_key=%22_design0%22`
    );
    ...
    const statuses_ddocs_res = await axios.get(
        `${base_url}/statuses/_all_docs?inclusive_end=false&start_key=%22_design%22&end_key=%22_design0%22`
    );
    ...
    const statuses_api_res = await axios.get(
        `${base_url}/statuses/_design/api`
    );
